...
class WeatherManager
    ...
    def request_weather_info(location)
        @location = location        
        @weather_daily = []
        response = RestAccuweatherService.instance.get_weather_info(location["Key"])
    ...
    def to_tts_string
        ...
        result_string += "It will be #{@weather_daily[0]["DayCondition"]} for most of the day. ".gsub("w/", "with")
        result_string += "And #{@weather_daily[0]["NightCondition"]} in the evening. ".gsub("w/", "with")
        result_string += "Tomorrow, temperature is (#{@weather_daily[1]["MinValue"]}) degree to (#{@weather_daily[1]["MaxValue"]}) degree. "
        result_string += "It will be #{@weather_daily[1]["DayCondition"]} for most of the day. ".gsub("w/", "with")
        result_string += "And #{@weather_daily[1]["NightCondition"]} in the evening. ".gsub("w/", "with")
        return result_string
    end
